...
static std::string next_token(int fd) {
    ...
    while (1) {
        ...
        if (next_char == '\0') {
            // IPC message terminator: skip so the parser tolerates null-separated
            // messages (rddlsim appends '\0' after each message).
            continue;
        }
...
static int do_node(const std::string& token, PSink& ps) {
    ...
    //   <?xml version="1.0" ...?>, <! ... >.  These carry no node and do not
    //   change nesting depth (signalled to parse_node via the -3 return code).
    if (node_tokens[0][0] == '?' || node_tokens[0][0] == '!') {
        return -3;
    }
...
static bool parse_node(int fd, PSink& ps) {
    ...
    while (!token.empty()) {
        if (token == "<") {
            ...
            if (delta == -3) {
                // XML declaration / processing instruction: consume its '>' and
                // keep reading (no node pushed, depth unchanged).
                token = next_token(fd);
                if (token != ">") {
                    ps.formaterror();
                    return false;
