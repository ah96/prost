...
> **Local fork — modified from upstream [prost-planner/prost](https://github.com/prost-planner/prost).**
> This copy contains a small **IPC-compatibility patch** so the planner
> interoperates with rddlsim builds whose `<no-header/>` session-init reply is
> **not** null-terminated. Without it, session initialisation fails with
> `Error: initializing session failed.` (the server then reports a premature
> end-of-file on the next round request).
>
> **Changes (rebuild with `./build.py` after applying):**
> - `src/search/ipc_client.cc` — the session request no longer sends
>   `<no-header/>`, so the server returns its full, `\0`-terminated reply
>   (XML declaration included) instead of taking the buggy no-header path.
> - `src/search/utils/strxml.cc` — the XML reader now (a) skips the `\0`
>   message terminator and (b) skips the `<?xml … ?>` declaration / processing
>   instructions, so it can parse the full reply.
>
