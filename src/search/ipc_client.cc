...
void IPCClient::initSession(string const& instanceName, string& plannerDesc) {
    ...
    os << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
       ...
       // Note: we intentionally do NOT request <no-header/>.  This rddlsim build's
       // no-header reply path omits the '\0' terminator (and clips one character),
       // which breaks session init.  Receiving the full, null-terminated message
       // (XML declaration included) is handled by the patched parser in strxml.cc.
       << "</session-request>" << '\0';
